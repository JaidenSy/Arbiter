...
An MCP server in NexusAI is a registered upstream that agents route tool calls through. NexusAI proxies JSON-RPC 2.0 requests to these servers, applies RBAC, injects vault secrets, and caches responses.
...
| `url` | string | yes | Base URL of the upstream MCP server. Must be reachable from the NexusAI container. |
...
The `url` is the base URL of the upstream MCP server. NexusAI appends the MCP path when forwarding. Examples:
...
Do not include a trailing path. NexusAI constructs the full request URL.

## The `cache_enabled` flag

This flag controls whether NexusAI caches responses from this server and serves cached results on future matching requests.
...
When `cache_enabled` is `false`, NexusAI skips all three cache layers (Redis, Postgres exact, Postgres semantic) on both reads and writes. The `cache_ttl_seconds` and `cache_similarity_threshold` fields are ignored.
...
When an agent calls `tools/list` on a server, NexusAI filters the returned tool list to only include tools the agent has permission to call. Agents never see tool names they are not permitted to use — this prevents information leakage about the server's capabilities.
