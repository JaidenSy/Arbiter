...
The `.env` file now contains `VAULT_ENCRYPTION_KEY` and Postgres credentials. Nothing GitHub- or Notion-specific.
...
Deploy Arbiter as the centralized MCP gateway. All 20+ agents register through it. All MCP servers register through it. Secrets rotation now happens in one place. Update the vault and the next tool call gets the new value. Permission changes for an agent happen through the Arbiter API, not by editing config files across 10 repos.
...
**Who they are**: A large enterprise (500+ employees) with an AI team deploying agents into production workflows: document processing, data analysis, internal tooling. The company is pursuing SOC2 Type II certification. Legal has flagged that AI agents calling external tools with customer credentials needs governance controls in place before the audit.
