...
NexusAI is not the only tool in this space. Here is an honest assessment of the alternatives — where they are strong, where they fall short, and when NexusAI is (and is not) the right choice.
...
- **No tool call semantic cache.** LiteLLM caches LLM completions. MCP tool call results — file reads, database queries, search results — are not cached. NexusAI caches at the tool response layer.
...
LiteLLM MCP support was added in late 2025 and is improving, but it is LLM-first tooling with MCP bolted on. NexusAI is MCP-first.

**When to use LiteLLM over NexusAI**: You need multi-provider LLM routing, spend tracking across model providers, or you're doing prompt caching across OpenAI/Anthropic/Gemini. NexusAI does not do any of that.
...
- **Semantic cache is for LLM completions, not tool calls.** Portkey caches the LLM response layer. NexusAI caches what the MCP server returned — file contents, query results, search hits. These are different things.
...
**When to use Portkey over NexusAI**: You want a managed SaaS LLM gateway with a polished trace UI across Claude, GPT-4, Gemini, and you don't need per-agent encrypted credentials or MCP tool call caching. Portkey's $49/month tier is a good value for LLM-layer observability.
...
**When to use Kong over NexusAI**: You are a large enterprise with an existing Kong deployment, a Kubernetes infrastructure team, and $5,000+/month API management budget. NexusAI is explicitly not competing at this price point and complexity level.
...
NexusAI on the free tier costs $0. On Pro it costs $29/month. The math is not close.

The counterargument for DIY: you own the code, you understand every decision, there is no dependency on an external project. That is a legitimate concern. NexusAI is MIT licensed and self-hostable — you can fork it, audit it, and never depend on the hosted version.

---

## Summary

| | NexusAI | LiteLLM | Portkey | Kong/Gravitee | DIY |
|---|---|---|---|---|---|
...
NexusAI's defensible position: the only MCP-native gateway with all four differentiators — per-agent encrypted vault, per-agent API key identity, tool call semantic cache, and single `docker compose up` deployment — at a price accessible to individual developers and small teams.
