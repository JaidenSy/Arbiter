...
Arbiter is not the only tool in this space. Here is an honest assessment of the alternatives: where they are strong, where they fall short, and when Arbiter is (and is not) the right choice.
...
- **No tool call semantic cache.** LiteLLM caches LLM completions. MCP tool call results (file reads, database queries, search results) are not cached. Arbiter caches at the tool response layer.
- **RBAC requires Enterprise ($250/month).** Free and open-source tiers have no RBAC. SSO and audit logs are also Enterprise-only.
- **`tools/list` not filtered by RBAC.** Tools not granted to an agent are still returned in `tools/list`, so agents can enumerate capabilities they cannot use.
...
LiteLLM MCP support was added in late 2025 and is improving, but it is LLM-first tooling with MCP bolted on. Arbiter is MCP-first.

**When to use LiteLLM over Arbiter**: You need multi-provider LLM routing, spend tracking across model providers, or you're doing prompt caching across OpenAI/Anthropic/Gemini. Arbiter does not do any of that.
...
- **Semantic cache is for LLM completions, not tool calls.** Portkey caches the LLM response layer. Arbiter caches what the MCP server returned: file contents, query results, search hits. These are different things.
...
**When to use Portkey over Arbiter**: You want a managed SaaS LLM gateway with a polished trace UI across Claude, GPT-4, Gemini, and you don't need per-agent encrypted credentials or MCP tool call caching. Portkey's $49/month tier is a good value for LLM-layer observability.
...
**When to use Kong over Arbiter**: You are a large enterprise with an existing Kong deployment, a Kubernetes infrastructure team, and $5,000+/month API management budget. Arbiter is explicitly not competing at this price point and complexity level.
...
DIY is the most common alternative. Most teams build this incrementally, a few env vars here, a home-grown permission check there, until they end up with a fragile undocumented auth system that no one fully understands.
...
Arbiter on the free tier costs $0. On Pro it costs $29/month. The math is not close.

The counterargument for DIY: you own the code, you understand every decision, there is no dependency on an external project. That is a legitimate concern. Arbiter is MIT licensed and self-hostable. Fork it, audit it, and never depend on the hosted version.

---

## Summary

| | Arbiter | LiteLLM | Portkey | Kong/Gravitee | DIY |
|---|---|---|---|---|---|
...
Arbiter's defensible position: the only MCP-native gateway with all four differentiators (per-agent encrypted vault, per-agent API key identity, tool call semantic cache, and single `docker compose up` deployment) at a price accessible to individual developers and small teams.
