...
class TestPlanLimitsConstants:
    def test_free_tier_max_agents(self):
        ...
        assert PLAN_LIMITS["free"]["max_agents"] == 3

    def test_free_tier_max_mcp_servers(self):
        from app.services.plan.plan_limits import PLAN_LIMITS
        assert PLAN_LIMITS["free"]["max_mcp_servers"] == 5

    def test_free_tier_max_tool_calls_mo(self):
        from app.services.plan.plan_limits import PLAN_LIMITS
        assert PLAN_LIMITS["free"]["max_tool_calls_mo"] == 1_000
    ...
    def test_all_enterprise_limits_are_none(self):
        """All four enterprise limit keys must be None (unlimited)."""
        from app.services.plan.plan_limits import PLAN_LIMITS
        for key, value in PLAN_LIMITS["enterprise"].items():
    ...
    def test_all_tiers_have_required_keys(self):
        from app.services.plan.plan_limits import PLAN_LIMITS
        required_keys = {"max_agents", "max_mcp_servers", "max_tool_calls_mo", "max_vault_secrets"}
...
class TestQuotaExceededError:
    def test_attributes_set_correctly(self):
        ...
        resets = datetime(2026, 5, 1, 0, 0, 0, tzinfo=timezone.utc)
    ...
    def test_str_includes_used_and_limit(self):
        from app.services.plan.plan_limits import QuotaExceededError
        resets = datetime(2026, 5, 1, tzinfo=timezone.utc)
        err = QuotaExceededError(resource="tool_calls", used=1050, limit=1000, resets_at=resets)
        msg = str(err)
        assert "1050" in msg
        assert "1000" in msg
