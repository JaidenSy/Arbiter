/**
 * NexusAI E2E — Settings page tests.
 *
...
test.describe('Settings — API Key', () => {
  test('save key → status turns Connected', async ({ page }) => {
    ...
    const stored = await page.evaluate(() => localStorage.getItem('nexusai_api_key'))
  ...
  test('clear key → status turns No key set', async ({ page }) => {
    ...
    await page.evaluate(() => localStorage.setItem('nexusai_api_key', 'nxai_testkey'))
    await page.reload()
