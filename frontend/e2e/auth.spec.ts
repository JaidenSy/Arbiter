...
test.describe('Login', () => {
  test('valid credentials → redirect to dashboard', async ({ page }) => {
    ...
    const email = process.env.E2E_EMAIL ?? 'e2e-user@nexusai.test'
  ...
  test('wrong password → error message shown', async ({ page }) => {
    await page.goto('/login')
    await page.getByLabel(/email/i).fill('wrong@nexusai.test')
...
test.describe('Logout', () => {
  ...
  test('logout clears tokens and redirects to login', async ({ page }) => {
    ...
    const token = await page.evaluate(() => localStorage.getItem('nexusai_access_token'))
